<template>
  <main class="pt-20">
    <section class="py-section px-margin-x-mobile md:px-margin-x max-w-container mx-auto">
      <div class="grid md:grid-cols-12 gap-8 items-center">
        <div class="md:col-span-5 reveal">
          <span class="font-label text-label text-warm-stone mb-4 block uppercase">{{ locale.t.aboulocale.t.subtitle }}</span>
          <h2 class="font-headline text-headline text-deep-forest mb-8 italic">{{ locale.t.aboulocale.t.title }}</h2>
          <p class="text-body-lg text-on-surface-variant leading-relaxed mb-6 font-light">
            {{ locale.t.aboulocale.t.story }}
          </p>
          <p class="text-body text-on-surface-variant leading-relaxed font-light italic">
            "{{ locale.t.aboulocale.t.quote }}"
          </p>
        </div>
    ...
    <section class="py-section bg-sage-wash px-margin-x-mobile md:px-margin-x max-w-container mx-auto">
      <h2 class="font-headline text-headline text-deep-forest mb-12 text-center reveal">{{ locale.t.aboulocale.t.facilitiesTitle }}</h2>
      <div class="grid md:grid-cols-3 gap-8">
        <div
          v-for="(facility, key) in locale.t.aboulocale.t.facilities"
          :key="key"
    ...
    <section class="py-section px-margin-x-mobile md:px-margin-x max-w-container mx-auto">
      <div class="grid md:grid-cols-12 gap-8 items-center">
        <div class="md:col-span-4 reveal">
          <h2 class="font-headline text-headline text-deep-forest mb-8">{{ locale.t.aboulocale.t.locationTitle }}</h2>
          <p class="text-body text-on-surface-variant font-light mb-8">{{ locale.t.aboulocale.t.locationAddress }}</p>
          <div class="space-y-4">
            <div class="flex items-start gap-4">
              ...
              <div>
                <p class="font-label text-label text-deep-forest uppercase">{{ locale.t.aboulocale.t.locationLabel }}</p>
                <p class="text-body font-light">{{ locale.t.aboulocale.t.locationAddress }}</p>
              </div>
            </div>
            <div class="flex items-start gap-4">
              <svg class="w-6 h-6 text-warm-stone flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <p class="font-label text-label text-deep-forest uppercase">{{ locale.t.aboulocale.t.hoursLabel }}</p>
                <p class="text-body font-light whitespace-pre-line">{{ locale.t.aboulocale.t.locationHours }}</p>
              </div>
            </div>
          </div>
        </div>
